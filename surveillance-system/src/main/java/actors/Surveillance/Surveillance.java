...
public class Surveillance extends AbstractBehavior<Surveillance.SurveillanceCommand> {
    ...
    private Surveillance(ActorContext<Surveillance.SurveillanceCommand> context, TimerScheduler<SurveillanceCommand> timers, SurveillanceServices surveillanceServices) {
        super(context);
        this.timers = timers;
        this.surveillanceServices = surveillanceServices;
        //Adapter to receive Messages from Receptionist
        this.receptionistAdater = getContext().messageAdapter(Receptionist.Listing.class, WrappedListingMessage::new);
        //Subscribe to Receptionist with Detector Key
        getContext().getSystem().receptionist().tell(Receptionist.subscribe(Detector.receptionist_detector_key, receptionistAdater));
    }
    ...
    private Behavior<SurveillanceCommand> onWrappedListing(WrappedListingMessage wrappedListing){
        ...
        this.detectorRefs = new HashSet<>(listing.getServiceInstances(Detector.receptionist_detector_key));
        return Behaviors.same();
    }
}