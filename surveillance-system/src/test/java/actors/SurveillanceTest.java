...
public class SurveillanceTest {
    ...
    private final TestProbe<Detector.DetectorCommand> detector = testKit.createTestProbe(Detector.DetectorCommand.class);
    ...
    public void setup() {
        doAnswer(invocationOnMock -> {
            ...
            return null;
        }).when(surveillanceServices).analyze(any(), any());

        testKit.system().receptionist().tell(Receptionist.register(Detector.receptionist_detector_key, detector.getRef()));
    }

    @Test
    public void surveillanceTestNoAlarm() {
        ActorRef<Surveillance.SurveillanceCommand> surveillance = testKit.spawn(Surveillance.create(surveillanceServices));

        LoggingTestKit.info("analyze").expect(
                testKit.system(),
                () -> {
