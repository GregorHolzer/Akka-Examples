...
public class DetectorTest {
    ...
    private final TestProbe<Surveillance.SurveillanceCommand> surveillance = testKit.createTestProbe(Surveillance.SurveillanceCommand.class);
    ...
    public void setup(){
        doAnswer(invocationOnMock -> {
            ...
            return null;
        }).when(detectorServices).alarmOn(any());

        doAnswer(invocationOnMock -> {
            ActorContext<Detector.DetectorCommand> context = invocationOnMock.getArgument(0);
            context.getLog().info("alarmOff");
            return null;
        }).when(detectorServices).alarmOff(any());

        doAnswer(invocationOnMock -> {
            ActorContext<Detector.DetectorCommand> context = invocationOnMock.getArgument(0);
            context.getLog().info("captureCamera");
            return null;
        }).when(detectorServices).cameraCapture(any(), any(), any());
        ...
        doAnswer(invocationOnMock -> {
            ...
            return null;
        }).when(detectorServices).detectPersons(any(), any());
    }
    ...
    public void detectorAlarmTest(){
        ...
        LoggingTestKit.info("detectPersons").expect(
                testKit.system(),
                () -> {
        ...
        LoggingTestKit.info("alarmOn").expect(
                testKit.system(),
                () -> {
        ...
        LoggingTestKit.info("alarmOff").expect(
                testKit.system(),
                () -> {
    ...
    public void detectorTimeoutTest(){
        ...
        LoggingTestKit.info("captureCamera").expect(
                testKit.system(),
                () -> {
                    //Wait for Timeout
                    return null;
